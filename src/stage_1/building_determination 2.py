...
def sub_rect(rectangle, id_count, img_id):
    rect = rectangle.copy()
    new_facede_2d = projection(img_id, rect)
    for i in range(len(new_facede_2d)):
        new_facede_2d[i][0] = new_facede_2d[i][0] + 350
        new_facede_2d[i][1] = new_facede_2d[i][1] + 165

        # calibrating_output = "../../image/" + img_id + "calibrated.jpg"
    name = "../../result/calibration_405_0035_00124454_" + str(id_count) + ".jpg"
    perspective(img_id, rectangle, new_facede_2d, name)
    id_count += 1
...
def order_rectangle_points(points):
    sorted_points = sorted(points, key=lambda p: (p[2], p[1]))  # Sort by Y, then by X

    top_left = sorted_points[-2]
...
def integration(rectangles, rect_index, img_id):

    mybbox = rectangles[rect_index]
    visual = [mybbox]
    ...
    img_name = "../../result3/calibration_" + img_id + "_" + rect_index + ".jpg"
...
if __name__ == '__main__':
    ...
    for rectangle in rectangles:
        ...
        name = "../../result_404/calibration_404_0030_00131330_"+str(id_count)+".jpg"
    ...
    #
    # building_file = f'../../import_Almere/3dbag_v210908_fd2cee53_4568.json'
    # j = import_cityjson(building_file)
    # id = "NL.IMBAG.Pand.0034100000061441"
    # arr = get_visibility_arrangement(j, id, '405_0035_00124455', "Almere")
#
#     facade_list = get_facades_list(j, id, 2.2)
#     visible_facade, visible_group = merge_co_planar(arr, facade_list)
#
#     for i in range(len(visible_group)):
#         facade_corner = get_facade_corner(visible_group, i)
#         dist = compute_distance(facade_corner[1], facade_corner[2])
#         if dist>5:
#             print(i)
#             print(dist)
#             print(facade_corner)
...
#
    # pt_list = []
    # file = f"../../import_Almere/NL.IMBAG.Pand.0034100000061441.json"
    # j = import_cityjson(file)
    # for each in j["CityObjects"]:
    #         if 'geometry' in j['CityObjects'][each]:
    #             for geom in j['CityObjects'][each]['geometry']:
    #                 print(geom["lod"])
    #                 if geom["lod"] == 2.2:
    #                     print(geom["boundaries"][0][8])
    #                     for i in geom["boundaries"][0][8][0]:
    #                         pt = j["vertices"][i]
    #                         x = float(pt[0]) * j["transform"]["scale"][0] + j["transform"]["translate"][0]
    #                         y = float(pt[1]) * j["transform"]["scale"][1] + j["transform"]["translate"][1]
    #                         z = float(pt[2]) * j["transform"]["scale"][2] + j["transform"]["translate"][2]
