...
if __name__ == '__main__':
    ...
    path = "/Users/yitongxia/Desktop/ams/camera.csv"

    # Open the input and output files
    with open(path, 'r') as infile, open('/Users/yitongxia/Desktop/ams/newnewnew.csv', 'w', newline='') as outfile:
        ...
        for row in reader:
            ...
            name = row[0].strip().split("_")[0]
            # Replace the first item in the row
            new_value = "I_" + name + ".jpg"
            row[0] = new_value
    ...
    path = f"../../visualization/Rotterdam_walls.off.off"
    non_wall_path = f"../../visualization/Rotterdam.off"
    ...
    img_id = '26_08033_r_rgb.tif'
    img_type = img_id.strip().split("_")[0]
    print(img_type)
    pts_groundtruthlist = []
    pts_original = []
    # id_count = len(rectangles)
    id_count = 0
    img_list = []

    images_folder = img_folder(f"../../../")
    print(images_folder)

    regis_model = registration_model()
    """after obtaining registration model in four directions:"""
    for img_id in images_folder:
        for rectangle in rectangles:
            rect = rectangle.copy()
            ...
            name = "../../test/calibration_" + img_id.strip().split(".")[0] + "_" + str(id_count) + ".jpg"
    ...
    cocofile = f'../../import_Almere/postprocessing3.json'
    ...
    json_object = json.dumps(j, indent=4)
    with open('postpro3.json', 'w') as json_file:
        json_file.write(json_object)
