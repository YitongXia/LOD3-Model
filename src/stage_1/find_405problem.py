...
def get_off_3Dfootprint(grouped_faces, vertices):
    ...
    return total_3Dfootprint, else_polygon, face_rect_pair
...
# def add_walls(j, filter_result, rectangles, face_rect_pair):
#     json_vertex_count = len(j['vertices'])
#     for each in j["CityObjects"]:
#         for geom in j["CityObjects"][each]['geometry']:
#             if geom["lod"] == 2.2:
...
def add_walls(j, filter_result, rectangles, face_rect_pair, vertices):
    json_vertex_count = len(j['vertices'])
    for each in j["CityObjects"]:
        for geom in j["CityObjects"][each]['geometry']:
            if geom["lod"] == 2.2:
                for i in filter_result:
                    polygons = face_rect_pair[i]
...
if __name__ == '__main__':
    ...
    non_wall_path = f"../../visualization/small_almere.off"
    vertices = read_mesh_vertex(path)
    faces, colors = read_mesh_faces_1(path)
    print("finish read")

    non_wall_vertices = read_mesh_vertex(non_wall_path)
    ...
    rectangles, else_polygon, face_rect_pair = get_off_3Dfootprint(grouped_faces, vertices)
    ...
    j = add_walls(j, filter_result, rectangles, face_rect_pair, vertices)
    j = write_non_wall(j, non_wall_surface, non_wall_vertices)
